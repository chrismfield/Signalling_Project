...
def step_setup():
    ...
    for section in Section.instances.values():
        for dectrig, direction in section.dectrig.values():
            if dec_trig_lookup[dectrig] in dec_trig_lookup:
    ...
    for section in Section.instances:
        for inctrig, direction in section.inctrig.values():
            # find corresponding dectrig section
            old_sections = dec_trig_lookup[inctrig][direction] # list of old_sections
            #put it all into the step_dict
            step_dict[inctrig]= {direction : {"old_berth_sections": old_sections, "new_berth_section": section}}
            if feeder_dict[section] in feeder_dict:
               feeder_dict[section].extend(old_sections)
            else:
                feeder_dict[section] = old_sections

    return step_dict, feeder_dict

def new_unknown_train(berth_section: Section):
    """used when a new train appears on the network"""
    Train.instances[Train.next_ID_counter] = Train()
    Train.instances[-1].berth_section = berth_section
    berth_section.trains.append(Train.instances[-1])
    Train.instances[-1].journey_log.append(berth_section.ref)
    #Add in a default headcode allocation, based on sequential from 1C00

def berth_set(trains:dict, berth_section: Section, headcode):
    """Used for initially setting a headcode & ID, before any train is in the berth"""
    # on mqtt berth set message
    Train.instances[Train.next_ID_counter] = Train()
    Train.instances[-1].berth_section = berth_section
    Train.instances[-1].headcode = headcode
    Train.headcode_lookup[headcode] = Train.instances[-1]
    berth_section.trains.append(Train.instances[-1])
    return
...
def train_clear(berth_section: Section = None, train: Train = None):
    ...
    train.berth_section.trains.pop(train)
...
def berth_calculate(feeder_dict, logger, mqtt_client):
    ...
    for section in Section.instances.values():
        ...
        if section.mode == "trackcircuit":
            if section.occstatus and not section.train:
                    for feeder in feeder_dict[section]:
                        if feeder.train and not feeder.occstatus:
                            berth_step(section, feeder)
            if not section.train:
