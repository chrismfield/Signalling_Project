...
class signal:
    def __init__(self,sigtype,address,ref,description,availableaspects,directionindicator):
...
def Addplunger(parent,existingref):
    ...
    def Writeplunger(parent):
        ...
        plungerdict[(plungerref.get())]=plunger(mode,address,ref,description)  # add an instance of an axlecounter to the list, with all the parameters.
        plungerlist(parent)
    ...
    #add in mode selector
    ttk.Label(plungersetupframe, text="Plunger Mode:").grid(column=0, row=2, sticky=W)   
    Radiobutton(plungersetupframe, text="Store request", variable=plungermode, value = 0).grid(column=1,row=2)
    Radiobutton(plungersetupframe, text="Don't store request", variable=plungermode, value = 1).grid(column=2,row=2)

    ttk.Label(plungersetupframe, text="Plunger Description:").grid(column=0, row=3, sticky=W)
    ttk.Entry(plungersetupframe, width=70, textvariable=plungerdescription).grid(column=1, columnspan=4, row=3, sticky=W) # plungerref entry

    ttk.Button(plungersetupframe, text="OK", command=lambda: Writeplunger(parent)).grid(column=4, row=10, sticky=E)    
...
def plungerlist(parent):
    ...
    def plungerlistwindow(parent):
        ...
        def plungerline(key,posi,windowtype):
            ...
            ttk.Label(framelist[windowtype], text="Mode: " +modedescription).grid(column = 2, row = posi, padx = 10)
            ttk.Label(framelist[windowtype], text="Description: "+plungerdict[key].description).grid(column = 3, row = posi, padx = 10)
            ttk.Button(framelist[windowtype], text="Edit", command= lambda: Addplunger(parent,key)).grid(column = 4, row = posi)
            ttk.Button(framelist[windowtype], text="Delete", command = lambda: deleteplunger(key,parent)).grid(column = 5, row = posi)
...
def Addsignal(parent,existingref):
    ...
    def Writesignal(parent):
        ...
        signaldict[(signalref.get())]=signal(sigtype,address,ref,description,availableaspects,directionindicator)  # add an instance of an signal to the list, with all the parameters. Need to add avalable aspects
        signallist(parent)
    ...
    listbox = Listbox(signalsetupframe, selectmode=MULTIPLE, height=4)
    listbox.grid(column=1, row = 3, sticky=W, pady=10)
    for item in ["Red", "Caution", "Clear", "Calling On"]:
...
def Addpoint(parent,existingref):
    ...
    def Writepoint(parent):
        ...
        pointdict[(pointref.get())]=point(mode,address,ref,description)  # add an instance of an axlecounter to the list, with all the parameters.
        pointlist(parent)
