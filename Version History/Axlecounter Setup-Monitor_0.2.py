...
class acframe: # axlecounter window class
    def __init__(self, parent, posi, rowposi, comport): #posi arguments allow multiple instances to be tiled
        ...
        addrst_entry = ttk.Entry(self.subframe, width=7, textvariable=addrst)
        addrst_entry.grid(column=2, row=0, sticky=(W, E))

        ttk.Label(self.subframe, textvariable=addrst).grid(column=2, row=1, sticky=(W, E))
        ttk.Button(self.subframe, text="Start", command=goserial1).grid(column=1, row=2, sticky=W)
...
def creatediags(parent,wins):
    comport = "COM6"
    print("Old list length: ", len(items))
    if wins < len(items):
        for i in range(len(items)):
            if (i+1) > wins:
                items[-1].subframe.destroy()
                del items[-1]
    if wins > len(items):
        for x in range(wins):
            if (x+1) > len(items):
                ...
                if x > 7:
                    y = 3
                    x = x - 8   
                items.append(acframe(parent,x,y,"COM6")) #this just adds a class with the same name to the list. 

    print("New list length: ", len(items))


class setupwindow: # need to change to have an add button and a remove button, and probably give each instace a unique reference. Could use the address? But what if it is necessary to change the address? Then it all goes wrong.
