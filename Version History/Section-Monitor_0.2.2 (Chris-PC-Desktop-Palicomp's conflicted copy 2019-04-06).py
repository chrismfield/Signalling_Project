...
def AddSection(root,existingref):
    ...
    def write_section(root):
        ...
        for selected in inclistbox.curselection():
            incselection.append(inclistbox.get(selected))
        for selected in declistbox.curselection():
            decselection.append(declistbox.get(selected))
...
# Set up Modbus devices - infact this is best done when creating or loading the asset. If necessary? Or just use the parameters?
...
def check_all_ACs():
    ...
    for AC in ACdict:
        try:
            slave = minimalmodbus.Instrument('COM1',AC.address) # --------------need to implement com port selector ---------------"
            AC.upcount, AC.downcount = slave.read_registers(13,2) # register number, number of registers
            slave.write_register(13, 0, 0) #register number, value, number of decimals
            slave.write_register(14, 0, 0) #register number, value, number of decimals
            status = "OK"
        except:
            status = "Comms failure with"+AC_to_check.ref
    return

def check_all_plungers():
    pass # -----------need to implement -------------

def movement_check(): # ---------need to consider how to make system work if downcount is a section increment ---------
    for section in sectiondict:
        section.occstatus += section.inctrig.upcount
        pass
    

# Check inputs - Axle Counters, Plungers and Points
...
def saveaslayout(root):
    ...
    if pickle_out is None: # asksaveasfile return `None` if dialog closed with "cancel".
        return  
    #pickle_out = open("layout.pickle","wb") #use this at a later date with defautl file opening
