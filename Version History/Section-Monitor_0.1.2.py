...
def AClist(parent):
    ...
    def deleteAC(key):
        if messagebox.askyesno("Delete Axle Counter","Delete Axle Counter "+key+" ?"):
            del ACdict[key]
            AClistdict[key].frame.destroy()
        pass

    class ACline:
        def __init__(self,parent,key,posi):
            ...
            ttk.Label(self.frame, text="Axle Counter Ref: "+key+" / Address: "+ACdict[key].address+" / Mode: " +modedescription+ " / Description: "+ACdict[key].description).grid(column = 0, row = posi)
            ttk.Button(self.frame, text="Edit", command= lambda: AddAC(parent,key)).grid(column = 1, row = posi)
            ttk.Button(self.frame, text="Delete", command = lambda: deleteAC(key)).grid(column = 2, row = posi)
    posi = 0
    for key in ACdict:
        AClistdict.update(key = ACline(AClistwindow,key,posi))
        posi += 1
    ttk.Button(AClistwindow, text="Add Axlecounter", command= lambda: AddAC(parent,"")).grid(column=0, row=500, sticky=E) # button to add an axlecounter
    return AClistwindow

def updateAClist(parent):
    AClistwindow = AClist(parent)
...
def main():
    ...
    ttk.Button(SectionMonitor, text="Add Axlecounter", command= lambda: AddAC(root,"")).grid(column=1, row=1, sticky=NW) # button to add a sectionline
    menubar = Menu(root)
    ...
    assetmenu.add_command(label="Axle counters", command=(AAA=lambda:AClistwindow(root)))
