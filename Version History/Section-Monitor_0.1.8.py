...
class route:
    def __init__(self, ref, description, mode, sections, points, signals):
        self.mode = mode #mode: with detection or without detection
        self.ref = ref # Freetext reference
        self.description = description # Freetext description
...
def updatesections(root): #use this to update the main monitoring window
    i = 0
    sectionlinelist = []
    for key, section in sectiondict:
        pass
        #sectionlinelist.append(sectionlinelist(key),section)
...
def sectionlist(parent):
    ...
    def sectionlistwindow(parent):
        ...
        def sectionline(key,posi,windowtype):
            ...
            ttk.Label(framelist[windowtype], text="Section ref: "+key).grid(column = 0, row = posi, padx = 10)
            ttk.Label(framelist[windowtype], text="Description: "+sectiondict[key].description).grid(column = 5, row = posi, padx = 10)
            ttk.Button(framelist[windowtype], text="Edit", command= lambda: AddSection(parent,key)).grid(column = 6, row = posi)
            ttk.Button(framelist[windowtype], text="Delete", command = lambda: deletesection(key,parent)).grid(column = 7, row = posi)
        ...
        ttk.Button(framelist[windowtype], text="Add section", command= lambda: AddSection(parent,"")).grid(column=0, columnspan=10, row=500, sticky=E) # button to add an axlecounter
...
def signallist(parent):
    ...
    def signallistwindow(parent):
        ...
        ttk.Button(framelist[windowtype], text="Add Signal",
...
def route_list(parent):
    ...
    def routelistwindow(parent):
        ...
        def routeline(key,posi,windowtype):
            ...
            ttk.Label(framelist[windowtype], text="Route ref: "+key).grid(column = 0, row = posi, padx = 10)            
            ttk.Label(framelist[windowtype], text="Mode: " +modedescription).grid(column = 4, row = posi, padx = 10)
            ttk.Label(framelist[windowtype], text="Description: "+routedict[key].description).grid(column = 5, row = posi, padx = 10)
            ttk.Button(framelist[windowtype], text="Edit", command= lambda: Add_route(parent,key)).grid(column = 6, row = posi)
            ttk.Button(framelist[windowtype], text="Delete", command = lambda: deleteroute(key,parent)).grid(column = 7, row = posi)
        ...
        ttk.Button(framelist[windowtype], text="Add route", command= lambda: Add_route(parent,"")).grid(column=0, columnspan=10, row=500, sticky=E) # button to add an axlecounter
...
def main():
    ...
    root.geometry("500x100") #Width x Height of main window
