...
class assetlist:
    ...
    def __init__(self,parent, windowtype):
        ...
        def AClist(parent,windowtype):
            ...
            posi = 0
            for key in ACdict:
                #AClistdict.update(key = ACline(assetlist.windowdict[windowtype],key,posi))
                ACline(assetlist.windowdict[windowtype],key,posi)
                posi += 1
            ttk.Button(self.windowdict[windowtype], text="Add Axlecounter", command= lambda: AddAC(parent,"")).grid(column=0, row=500, sticky=E) # button to add an axlecounter
