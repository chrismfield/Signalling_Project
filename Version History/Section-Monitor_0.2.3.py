...
def AddSection(root,existingref):
    ...
    def write_section(root):
        ...
        for selected in inclistbox.curselection():
            incselection.append((inclistbox.get(selected),"upcount")) #--------------add ability to select whether upcount or downcount increments
        for selected in declistbox.curselection():
            decselection.append((declistbox.get(selected),"upcount")) #----------------ditto above decrements
    ...
    try:
        for x in range(inclistbox.size()):
            if inclistbox.get(x) in sectiondict[existingref].inctrig:
    ...
    try:
        for x in range(declistbox.size()):
            if declistbox.get(x) in sectiondict[existingref].dectrig:
...
def AClist(parent):
    ...
    def AClistwindow(parent):
        ...
        def ACline(key,posi,windowtype):
            ...
            ttk.Label(framedict[windowtype], text="Description: "+ACdict[key].description).grid(column = 4, row = posi, padx = 10)
            ttk.Button(framedict[windowtype], text="Edit", command= lambda: AddAC(parent,key)).grid(column = 5, row = posi)
            ttk.Button(framedict[windowtype], text="Delete", command = lambda: deleteAC(key,parent)).grid(column = 6, row = posi)
...
def plungerlist(parent):
    ...
    def plungerlistwindow(parent):
        ...
        def plungerline(key,posi,windowtype):
            ...
            ttk.Label(framedict[windowtype], text="Description: "+plungerdict[key].description).grid(column = 6, row = posi, padx = 10)
            ttk.Button(framedict[windowtype], text="Edit", command= lambda: Addplunger(parent,key)).grid(column = 7, row = posi)
            ttk.Button(framedict[windowtype], text="Delete", command = lambda: deleteplunger(key,parent)).grid(column = 8, row = posi)
...
def Addsignal(parent,existingref):
    ...
    for aspect in aspectvariables:
            Checkbutton(signalsetupframe, text=aspect[2]+":", command=toggle, variable=aspect[0],anchor=W).grid(row = aspect[3], column = 1, sticky=W)
    ...
    ttk.Entry(signalsetupframe, width=70, textvariable=signaldescription).grid(column=1, columnspan=4, row=15, sticky=W) # signalref entry
...
def check_all_ACs():
    ...
    for ACkey, ACinstance in ACdict.items():
        try:
            ...
            ACinstance.upcount, ACinstance.downcount = slave.read_registers(13,2) # register number, number of registers
...
def saveaslayout(root):
    pickle_out = filedialog.asksaveasfile(mode='wb', defaultextension=".lyt")
    if pickle_out is None: # asksaveasfile return `None` if dialog closed with "cancel".
        return  
...
#    pickle.dump(RS485port, pickle_out)
    pickle_out.close() # close the file
    global currentfile
    currentfile = os.path.basename(pickle_out.name)
    root.title("Section Monitor - "+ currentfile)
    print("saved")

def savelayout():
    global currentfile
    pickle_out = open(currentfile,"wb")
    pickle.dump(sectiondict, pickle_out)
    pickle.dump(ACdict, pickle_out)
    pickle.dump(signaldict, pickle_out)
    pickle.dump(plungerdict, pickle_out)
...
def loadlayout(root):
    ...
    global currentfile
    global RS485port
    pickle_in = filedialog.askopenfile(mode='rb', defaultextension=".lyt")
    if pickle_in is None: # asksaveasfile return `None` if dialog closed with "cancel".
        return  
...
#    RS485port = pickle.load(pickle_in)
    currentfile = os.path.basename(pickle_in.name)
    print("loadnow")
    updatesections(root)
    root.title("Section Monitor - "+ currentfile)
    pass

def default_load(root, file="default.lyt"):
    global sectiondict
    global ACdict
    global signaldict
    global plungerdict
...
def control_frame(root):
    ...
    Checkbutton(ControlFrame, text="Run System", command=run_system, variable=runvartk, anchor=W).grid(row = 0, column = 0, sticky=W)
...
def main():
    ...
    filemenu.add_command(label="Save As...", command=lambda:saveaslayout(root))
