...
class ACFrame:  # axlecounter window class
    ...
    def __init__(self, parent, posi, rowposi):  # posi arguments allow multiple instances to be tiled
        ...
        ttk.Button(self.subframe, text="Start", command=lambda: self.goserial1()).grid(columnspan=2, column=3, row=0, sticky=W)
        ttk.Button(self.subframe, text="Clear Count", command=self.goserialclr1).grid(columnspan=2, rowspan=2, column=3, row=3, sticky=W)
        ...
        ttk.Button(self.subframe, text="Read Parameters", command=lambda: self.readparameters()).grid(columnspan=5,
        ...
        ttk.Button(self.subframe, text='<', width=2, command=lambda: self.adjustparameters(1, -5)).grid(column=3, row=6,
                                                                                                        sticky=W)
        ttk.Button(self.subframe, text='>', width=2, command=lambda: self.adjustparameters(1, 5)).grid(column=4, row=6,
                                                                                                       sticky=W)
    ...
    def goserial1(self, *args):
        try:
            ...
            print(datetime.datetime.now().strftime("%H:%M:%S") + " Comms failed with module address "+ str(self.addrst.get()))
        self.parent.after(500, self.goserial1)
        return

    def goserialclr1(self, *args):
        try:
            axlecounter = minimalmodbus.Instrument(comport, int(self.addrst.get()))  # update comport and slave address
...
class setupwindow:  # need to change to have an add button and a remove button, and probably give each instace a unique
    ...
    def __init__(self, master):
        ...
        NoOfCounters.grid(column=1, row=1, sticky=W)
        ...
        for child in setupframe.winfo_children(): child.grid_configure(padx=5, pady=5)
